...
DROP VIEW DeskView
GO
CREATE OR ALTER VIEW DeskView AS
	SELECT *
	FROM Desk D
	WHERE D.LengthInCm >100
GO

-- a view with a SELECT statement that operates on at least 2 different tables and contains at least one JOIN operator
DROP VIEW BossView
GO
CREATE OR ALTER VIEW BossView AS
	select B.Surname
	from Boss as B
	inner join Secretaries S on B.SecretaryId = S.SecretaryID
GO
...
CREATE OR ALTER VIEW ProductView AS
	SELECT PM.PrimeMatterId,PM.NameOfTheMatter, F.FactoryName
	FROM PrimeMatter PM
	INNER JOIN CreatedProduct CP ON PM.PrimeMatterId = CP.PrimeMatterId
	INNER JOIN Factory F ON CP.FactoryId = F.FactoryId
	GROUP BY PM.PrimeMatterId ,PM.NameOfTheMatter, F.FactoryName
GO

--Test 1
EXEC insertIntoViews 'DeskView'
EXEC insertIntoTests 'test1', 1
EXEC insertIntoTables 'Desk'
EXEC connectTableToTest 'Desk', 'test1',500,1
EXEC connectViewToTest 'DeskView', 'test1'

--Test 2
-- a table with a single-column primary key and a foreign keys 
EXEC insertIntoViews 'BossView'
EXEC insertIntoTests 'test2', 2
EXEC insertIntoTables 'Secretaries'
EXEC connectTableToTest 'Secretaries', 'test2', 500, 1
EXEC insertIntoTables 'Boss'
EXEC connectTableToTest 'Boss', 'test2', 500 ,2
EXEC connectViewToTest 'BossView', 'test2'


--TEST 3
-- a table with a multicolumn primary key : PrimeMatterId

EXEC insertIntoViews 'ProductView'
EXEC insertIntoTests 'test3', 3
EXEC insertIntoTables 'PrimeMatter'
EXEC connectTableToTest 'PrimeMatter', 'test3', 500, 1
EXEC insertIntoTables 'Factory'
EXEC connectTableToTest 'Factory', 'test3', 500, 2
EXEC insertIntoTables 'CreatedProduct'
EXEC connectTableToTest 'CreatedProduct', 'test3', 500, 2
EXEC connectViewToTest 'ProductView', 'test3'
